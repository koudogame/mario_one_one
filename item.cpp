...
void Item::getGoal( PlayerData PlayerData )
{
    for( int i = 0; i < height_; i++ )
    {
        for( int j = 0; j < width_; j++ )
        {
            ...
            item_[ 0 ][ i ][ j ]->getGoal( PlayerData.goal_flag );
        }
    }
}

void Item::getEnd( PlayerData PlayerData )
{
    for( int i = 0; i < height_; i++ )
    {
        for( int j = 0; j < width_; j++ )
        {
            if( item_[ 0 ][ i ][ j ]->getId() == kFlag )
                item_[ 0 ][ i ][ j ]->upHata( PlayerData.end_flag );
        }
    }
}
